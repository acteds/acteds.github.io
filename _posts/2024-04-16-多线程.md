...
3. `notify()`：唤醒在锁对象上等待的单个线程。

4. `notifyAll()`：唤醒在锁对象上等待的所有线程。

5. `ReentrantLock`类：可重入锁，提供了更灵活的锁定机制，可以替代`synchronized`关键字。常用方法包括`lock()`、`unlock()`、`tryLock()`等。

6. `Condition`接口：配合`ReentrantLock`使用，提供了类似`wait()`和`notify()`的功能，但更加灵活。

7. `ReadWriteLock`接口：读写锁，提供了读锁和写锁，可以提高读操作的并发性能。

8. `StampedLock`类：提供了一种乐观读锁的机制，适用于读多写少的场景。
