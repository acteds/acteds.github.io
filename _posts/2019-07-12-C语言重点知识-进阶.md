...
&emsp;&emsp;**void\***可以接受任意类型的地址,但不能进行解引用操作.也不能进行加减运算.  
# 字符串
...
&emsp;&emsp;该方式为声明结构体**student**时定义结构体变量**s1**  
...
&emsp;&emsp;该函数有多种模式，其中**r+**和**w+**看似一样，都是读写其实还是有几点区别的；  
...
&emsp;&emsp;这样是合法的,这两个字符串会被当成一个字符串:  
